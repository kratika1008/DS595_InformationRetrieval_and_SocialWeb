...
MY_ANUM  = 123456789 # put your UID here
...
class BetterIndex(object):
    ...
    def wildcard_search_or(self, text):
        ...
        docs_index = []
        for t in terms:
            permuterms = self._rotate(t)
            docs_index = list(set(docs_index + list(crawl_tree(self._bt.root,permuterms))))
        docs=[self._documents[i] for i in docs_index]
        if len(docs)==0:
            docs.append('Term does not exist in any document')
        return docs
    ...
    def wildcard_search_and(self, text):
        ...
        for i,t in enumerate(terms):
            permuterms = self._rotate(t)
            if i==0:
                docs_index = crawl_tree(self._bt.root,permuterms)
            docs_index = list(crawl_tree(self._bt.root,permuterms) & set(docs_index))
        docs=[self._documents[i] for i in docs_index]
        if len(docs)==0:
            docs.append('Term does not exist in any document')
        return docs
